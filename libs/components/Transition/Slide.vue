<script lang="ts">
import Vue from 'vue';
import { SlideTransition, SlideGroup } from './@types';
...
export default Vue.extend({
  ...
  props: {
    enter: {
      type: Object as () => SlideTransition,
      required: false,
      default: () => ({ active: false } as SlideTransition),
    },
    leave: {
      type: Object as () => SlideTransition,
      required: false,
      ...
      on: {
        beforeEnter(el: HTMLElement) {
          if (ctx.props.enter.active === true) {
            ...
            const inverse =
              typeof ctx.props.enter.inverse === 'boolean'
                ? ctx.props.enter.inverse
                : DEFAULT_ENTER_INVERSE;

            const transform =
              ctx.props.enter.vertical === true ? 'translateY' : 'translateX';

            el.style.opacity = '0';
            el.style.transform =
              inverse === false ? `${transform}(-30px)` : `${transform}(30px)`;
        ...
        async leave(el: HTMLElement, done: Function) {
          if (ctx.props.leave.active === true) {
            ...
            const inverse =
              typeof ctx.props.leave.inverse === 'boolean'
                ? ctx.props.leave.inverse
                : DEFAULT_LEAVE_INVERSE;

            const transform =
              ctx.props.leave.vertical === true ? 'translateY' : 'translateX';

            el.style.opacity = '0';
            el.style.transform =
              inverse === true ? `${transform}(-30px)` : `${transform}(30px)`;
